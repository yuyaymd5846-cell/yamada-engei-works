...
async function main() {
    console.log('Seeding pesticide rotations...')

    for (const rotation of PESTICIDE_ROTATIONS) {
        const existing = await prisma.pesticideRotation.findFirst({
            where: { label: rotation.label }
        })

        if (!existing) {
            await prisma.pesticideRotation.create({
                data: {
                    label: rotation.label,
                    pesticides: JSON.stringify(rotation.pesticides)
                }
            })
            console.log(`Created rotation ${rotation.label}`)
        } else {
            console.log(`Rotation ${rotation.label} already exists, skipping.`)
        }
    }
}
